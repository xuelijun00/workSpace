...
function getUrl(type){//拼接url
	...
	if(type === 1){ 	
		return contextPath + '/report/ebay_domestic/sku/gridEbay?business=Ebay_US&startDate=' + startDate + "&endDate=" + endDate+ "&skuOld=" + oldsku+ "&sku=" + sku;
	}else{
		return contextPath + '/report/ebay_domestic/sku/chartEbay?business=Ebay_US&startDate=' + startDate + "&endDate=" + endDate+ "&skuOld=" + oldsku+ "&sku=" + sku;
	}
}
...
function exportData(){
	...
	var chartUrl =  contextPath + '/report/ebay_domestic/sku/gridEbay?business=Ebay_US&startDate=' + startDate + "&endDate=" + endDate+ "&skuOld=" + oldsku+ "&sku=" + sku;	
	$.ajax({
		url : chartUrl,
		cache : false,
		type:"get",
...
function getChartData(chartUrl){
	...
	return {
		title:{text:"Ebay业务线美仓时间段sku销售数据"}
		,categories:sku
		,y:y
