...
function getUrl(type){//拼接url
	...
	if(type === 1){
		return contextPath + '/report/sales_performance/grid?business=Ebay_AU&startDate=' + startDate + "&endDate=" + endDate;
	}else{
		return contextPath + '/report/sales_performance/chart?business=Ebay_AU&startDate=' + startDate + "&endDate=" + endDate;
	}
}
function queryData(){
...
function exportData(){
	var startDate = $("#start_date").val();
	var endDate = $("#end_date").val();
	var fileName = "Ebay海外仓澳洲仓业绩数据" + startDate +"-"+ endDate + ".csv";
