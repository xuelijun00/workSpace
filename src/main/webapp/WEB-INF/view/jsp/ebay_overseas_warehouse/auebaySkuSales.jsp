...
function getUrl(type){//拼接url
	...
	if(type === 1){ 	
		return contextPath + '/report/ebay_domestic/sku/gridEbay?business=Ebay_AU&startDate=' + startDate + "&endDate=" + endDate+ "&skuOld=" + oldsku+ "&sku=" + sku;
	}else{
		return contextPath + '/report/ebay_domestic/sku/chartEbay?business=Ebay_AU&startDate=' + startDate + "&endDate=" + endDate+ "&skuOld=" + oldsku+ "&sku=" + sku;
	}
}
...
function exportData(){
	...
	var startDate = $("#start_date").val();
	var endDate = $("#end_date").val();
	var fileName = "Ebay业务线澳洲仓每日sku销售数据" + startDate +"-"+ endDate + ".csv";
	var title = [ 'sku', '原始sku', '日期（day）', '订单数' ,'数量' ,'订单金额_美元'];
	var column = ['sku','skuOld','reportDate','orders','quantity','sales'];
	var chartUrl =  contextPath + '/report/ebay_domestic/sku/gridEbay?business=Ebay_AU&startDate=' + startDate + "&endDate=" + endDate+ "&skuOld=" + oldsku+ "&sku=" + sku;	
	$.ajax({
		url : chartUrl,
		cache : false,
		type:"get",
