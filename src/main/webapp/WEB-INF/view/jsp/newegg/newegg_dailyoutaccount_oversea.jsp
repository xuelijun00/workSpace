...
<div class="wrapper wrapper-content">
    <div class="ibox-title"><h5>新蛋账号维度每日发货数据(海外仓)(注意：本页面货币的单位，均为人民币（元）)</h5></div>
...
function getUrl(){
	...
	var url = "";
		url = contextPath + '/report/daily_out_account_oversea/grid?platform=new_Newegg&startDate=' + startDate + "&endDate=" + endDate;
	if(account !== ''){
...
function exportData(){
	...
	var startDate = $("#start_date").val();
	var endDate = $("#end_date").val();
	var fileName = "新蛋账号维度每日发货数据(海外仓)" + startDate +"-"+ endDate + ".csv";
...
function typeChangeName(){
	var	warehouseType = $("#warehouseType").val();
	var url = contextPath + '/report/daily_out_account_oversea/warehouseName?platform=new_Newegg';
...
(function(){
	...
	$.ajax({
		url : contextPath + "/report/daily_out_account_oversea/accounts?platform=new_Newegg",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
	...
	$.ajax({
		url : contextPath + "/report/daily_out_account_oversea/warehouseType?platform=new_Newegg",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
	...
	$.ajax({
		url : contextPath + "/report/daily_out_account_oversea/warehouseName?platform=new_Newegg",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
