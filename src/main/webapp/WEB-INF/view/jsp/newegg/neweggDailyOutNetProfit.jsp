...
<div class="wrapper wrapper-content">
    <div class="ibox-title"><h5>新蛋发货订单净利明细(注意：本页面货币的单位，均为人民币（元）)</h5></div>
...
function exportData(){
	...
	var endDate = $("#end_date").val();
	var	platform = $("#platform").val();
	var fileName = "新蛋发货订单净利明细" + startDate +"-"+ endDate +"-"+ platform + ".csv";
...
function palAndAcc(){
	...
	$.ajax({
		url : contextPath + "/report/profit_details/newEgg/account?platform=" + platform,
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
...
(function(){
	...
	$.ajax({
		url : contextPath + "/report/profit_details/newEgg/zhuzhandian",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
	...
	$.ajax({
		url : contextPath + "/report/profit_details/newEgg/account",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
