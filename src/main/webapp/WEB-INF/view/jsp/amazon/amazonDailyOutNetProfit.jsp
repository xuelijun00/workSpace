...
<div class="wrapper wrapper-content">
    <div class="ibox-title"><h5>Amazon SKU净利明细(注意：本页面货币的单位，只有柱状图（税后综合净利）是美元，其他均为人民币（元）)</h5></div>
    ...
    <form class="form-inline">
        ...
        <h2 class="text-center">Amazon税后综合净利（美元）和税后综合利润率（按日期汇总）</h2>
		<div class="hr-line-dashed"></div>
...
function getUrl(type){
	...
	var url = "";
		...
		if(type === 1){
			url = contextPath + '/report/profit_details/grid?platform=amazon&startDate=' + startDate + "&endDate=" + endDate;
		}else{
			url = contextPath + '/report/profit_details/chart?platform=amazon&startDate=' + startDate + "&endDate=" + endDate;
		}
...
function exportData(){
	...
	var fileName = "Amazon SKU净利明细" + startDate +"-"+ endDate + ".csv";
...
(function(){
	...
	$.ajax({
		url : contextPath + "/report/profit_details/zhuzhandian?platform=amazon",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
	...
	$.ajax({
		url : contextPath + "/report/profit_details/account?platform=amazon",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
