...
function getUrl(type){//拼接url
	...
	if(type === 1){
		return contextPath + '/report/sales_performance/grid?business=wish&startDate=' + startDate + "&endDate=" + endDate;
	}else{
		return contextPath + '/report/sales_performance/chart?business=wish&startDate=' + startDate + "&endDate=" + endDate;
	}
}
function queryData(){
...
function exportData(){
	var startDate = $("#start_date").val();
	var endDate = $("#end_date").val();
	var fileName = "wish业务线每日销售数据" + startDate +"-"+ endDate + ".csv";
