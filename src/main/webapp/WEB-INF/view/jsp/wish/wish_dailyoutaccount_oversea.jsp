...
<div class="wrapper wrapper-content">
    <div class="ibox-title"><h5>Wish账号维度每日发货数据(海外仓)(注意：本页面货币的单位，均为人民币（元）)</h5></div>
...
function getUrl(){
	...
	var url = "";
		url = contextPath + '/report/daily_out_account_oversea/grid?platform=wish&startDate=' + startDate + "&endDate=" + endDate;
	if(account !== ''){
...
function typeChangeName(){
	var	warehouseType = $("#warehouseType").val();
	var url = contextPath + '/report/daily_out_account_oversea/warehouseName?platform=wish';
...
(function(){
	...
	$.ajax({
		url : contextPath + "/report/daily_out_account_oversea/accounts?platform=wish",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
	...
	$.ajax({
		url : contextPath + "/report/daily_out_account_oversea/warehouseType?platform=wish",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
	...
	$.ajax({
		url : contextPath + "/report/daily_out_account_oversea/warehouseName?platform=wish",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
