...
function getUrl(type){//拼接url
	...
	return contextPath + '/report/dailyBuyerCountrySales/grid?platform=lazada&startDate=' + startDate + "&endDate=" + endDate + "&buyerCountry=" + buyerCountry;
	/* } */
}
function queryData(){	
...
function exportData(){
	var startDate = $("#start_date").val();
	var endDate = $("#end_date").val();
	var fileName = "lazada买家国家交易数据" + startDate +"-"+ endDate + ".csv";
...
(function(){
	...
	$.ajax({
		url : contextPath + "/report/dailyBuyerCountrySales/buyerCountry?platform=lazada",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
