...
function getUrl(type){//拼接url
	...
	if(type === 1){
		return contextPath + '/report/sales_performance/grid?startDate=' + startDate + "&endDate=" + endDate + "&business=" + business;
	}else{
		return contextPath + '/report/sales_performance/chart?startDate=' + startDate + "&endDate=" + endDate + "&business=" + business;
	}
}
function queryData(){
...
function exportData(){
	var startDate = $("#start_date").val();
	var endDate = $("#end_date").val();
	var fileName = "各业务线每日销售数据" + startDate +"-"+ endDate + ".csv";
...
(function(){
	...
	common.grid({
		...
		/* ,gridComplete:function(a,d,c){
			var salesSumValue = $("#list2").getCol('sales', false, 'sum');
			var ordersSumValue = $("#list2").getCol('orders', false, 'sum');
			$("#list2").footerData('set', {'business':'合计：','sales': salesSumValue,'orders': ordersSumValue}); 
		} */
	});
