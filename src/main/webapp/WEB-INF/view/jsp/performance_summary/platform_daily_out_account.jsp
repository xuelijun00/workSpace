...
<div class="wrapper wrapper-content">
    <div class="ibox-title"><h5>各平台账号维度每日发货数据(注意：本页面货币的单位，均为人民币（元）)</h5></div>
...
function getUrl(){
	...
	var url = "";
		...
		url = contextPath + '/report/daily_out_account/grid?startDate=' + startDate + "&endDate=" + endDate;
...
function palChangeAcc(){
	...
	$.ajax({
		url : contextPath + "/report/daily_out_account/accounts?platform=" + platform,
		cache : false,
		type:"get",
		async: false,
...
function exportData(){
	...
	var title = [ '平台名称', '账号', '管理员', '发货单数', '客单价', '发货收入（元）', '退款', '成本', '毛利',
	...
	/* var title = [ '报表时间', '平台名称', '账号', 'sku','管理员','发货单数','平均价','发货收入（元）','退款','成本','毛利','运费','平台费用','包材费','订单执行费','运营费','边际利润','税后综合净利'];
...
(function(){
	...
	$.ajax({
		url : contextPath + "/report/daily_out_account/platforms",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
	...
	$.ajax({
		url : contextPath + "/report/daily_out_account/accounts",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
	...
	common.grid({
		title:"各平台账号维度每日发货数据"
		,url:getUrl()
		,colNames:[ '平台名称', '账号', '管理员', '发货单数', '客单价', '发货收入（元）', '退款', '成本', '毛利',
			'运费', '平台费用', '包材费', '订单执行费', '运营费', '边际利润', '税前综合净利', '税后综合净利', '报表时间', '分类/主站点']
