...
<div class="wrapper wrapper-content">
    <div class="ibox-title"><h5>沃尔玛账号维度每日发货数据(注意：本页面货币的单位，均为人民币（元）)</h5></div>
...
function getUrl(){
	...
	var url = "";
		url = contextPath + '/report/daily_out_account/grid?platform=walmart&startDate=' + startDate + "&endDate=" + endDate;
	if(account !== ''){
...
function exportData(){
	...
	var startDate = $("#start_date").val();
	var endDate = $("#end_date").val();
	var fileName = "沃尔玛账号维度每日发货数据" + startDate +"-"+ endDate + ".csv";
...
(function(){
	...
	$.ajax({
		url : contextPath + "/report/daily_out_account/accounts?platform=walmart",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
