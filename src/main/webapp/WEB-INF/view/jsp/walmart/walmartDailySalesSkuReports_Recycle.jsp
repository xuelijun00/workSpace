...
<div class="wrapper wrapper-content">
    <div class="ibox-content">
        ...
        <h2 class="text-center">沃尔玛业务线sku环比增长数据折线图</h2>
...
function getUrl(type){//拼接url
	...
	if(type === 1){ 	
		return contextPath + '/report/sku_recycle/walmart/grid?&startDate=' + startDate + "&endDate=" + endDate+ "&skuOld=" + oldsku+ "&sku=" + sku;
	}else{
		return contextPath + '/report/sku_recycle/walmart/chart?&startDate=' + startDate + "&endDate=" + endDate+ "&sku=" + sku;
	}
}
...
/* 	var sku = encodeURIComponent($('#sku').val());
	...
	var fileName = "沃尔玛业务线sku环比增长数据" + startDate +"-"+ endDate + ".csv";
	var title = [ '平台/业务线', 'sku', '原始sku', '日期（day）', '订单数' ,'数量' ,'订单金额_美元'];
	var column = [ 'business', 'sku','skuOld','reportDate','orders','quantity','sales'];
	/* var gridUrl =  contextPath + '/report/sku_recycle/grid?business=wish&startDate=' + startDate + "&endDate=" + endDate+ "&skuOld=" + oldsku+ "&sku=" + sku;	 */
	$.ajax({
		url : getUrl(1),
		cache : false,
		type:"get",
