...
<div class="wrapper wrapper-content">
    <div class="ibox-title"><h5>SMT SKU净利明细(注意：本页面货币的单位，只有柱状图（税后综合净利）是美元，其他均为人民币（元）)</h5></div>
...
function getUrl(type){
	...
	var url = "";
	if(type === 1){
		url = contextPath + '/report/profit_details/grid?platform=smt&startDate=' + startDate + "&endDate=" + endDate;
	}else{
		url = contextPath + '/report/profit_details/chart?platform=smt&startDate=' + startDate + "&endDate=" + endDate;
	}
...
function exportData(){
	...
	var fileName = "SMT SKU净利明细" + startDate +"-"+ endDate + ".csv";
...
(function(){
	...
	$.ajax({
		url : contextPath + "/report/profit_details/zhuzhandian?platform=smt",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
	...
	$.ajax({
		url : contextPath + "/report/profit_details/account?platform=smt",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
