...
<div class="wrapper wrapper-content">
    <div class="ibox-title"><h5>Ebay站点专线发货业绩（注意：本页面货币的单位，均为人民币（元）） </h5></div>
...
function exportGridData(){
	var channelName = $("#site1").val();
	var startDate = $("#start_date1").val();
	var endDate = $("#end_date1").val();
	var fileName = "Ebay站点专线发货业绩 " + startDate + "-" + endDate + "-" + channelName + ".csv";
	var title = [ '日期','专线', '发货单数', '客单价', '发货收入（元）', '退款', '成本', '毛利', '运费', '平台费用', '包材费', '订单执行费', '运营费', '边际利润', '税后综合净利', '税后综合利润率'];
...
(function(){
	...
	common.grid({
		...
		title:"Ebay站点专线发货业绩 "
		,url:getUrl()
		,colNames:[ '日期','专线', '发货单数', '客单价', '发货收入（元）', '退款', '成本', '毛利', '运费', '平台费用', '包材费', '订单执行费', '运营费', '边际利润', '税后综合净利', '税后综合利润率']
