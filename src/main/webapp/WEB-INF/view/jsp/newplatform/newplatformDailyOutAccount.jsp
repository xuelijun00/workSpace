...
<div class="wrapper wrapper-content">
    <div class="ibox-title"><h5>新平台账号维度每日发货数据(注意：本页面货币的单位，均为人民币（元）)</h5></div>
...
function getUrl(){
	...
	var url = "";
		...
		url = contextPath + '/report/daily_out_account/newPlatform/grid?startDate=' + startDate + "&endDate=" + endDate;
...
function palChangeAcc(){
	...
	$.ajax({
		url : contextPath + "/report/daily_out_account/newPlatform/accounts?platform=" + platform,
		cache : false,
		type:"get",
		async: false,
...
(function(){
	...
	$.ajax({
		url : contextPath + "/report/daily_out_account/newPlatform/platforms",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
	...
	$.ajax({
		url : contextPath + "/report/daily_out_account/newPlatform/accounts",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
