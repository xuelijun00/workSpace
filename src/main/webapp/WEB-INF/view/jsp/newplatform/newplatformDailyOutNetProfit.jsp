...
<div class="wrapper wrapper-content">
    <div class="ibox-title"><h5>新平台SKU净利明细(注意：本页面货币的单位，只有柱状图（税后综合净利）是美元，其他均为人民币（元）)</h5></div>
	...
	<div class="hr-line-dashed"></div>
	      <h2 class="text-center">新平台税后综合净利（美元）和税后综合利润率（按日期汇总）</h2>
	<div class="hr-line-dashed"></div>
...
function getUrl(type){
	...
	var url = "";
	if(type === 1){
		url = contextPath + '/report/profit_details/newPlatform/grid?startDate=' + startDate + "&endDate=" + endDate;
	}else{
		url = contextPath + '/report/profit_details/newPlatform/chart?startDate=' + startDate + "&endDate=" + endDate;
	}
...
function exportData(){
	...
	var fileName = "新平台SKU净利明细" + startDate +"-"+ endDate +"-"+ platform + ".csv";
...
function palAndAcc(type){
	...
	$.ajax({
		url : contextPath + "/report/profit_details/newPlatform/account?platform=" +platform,
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
...
(function(){
	...
	$.ajax({
		url : contextPath + "/report/profit_details/newPlatform/platform",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
	...
	$.ajax({
		url : contextPath + "/report/profit_details/newPlatform/zhuzhandian",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
	...
	$.ajax({
		url : contextPath + "/report/profit_details/newPlatform/account",
		cache : false,
		type:"get",
		async: false,
		success : function(data) {
