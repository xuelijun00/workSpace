...
<div class="wrapper wrapper-content">
    <div class="ibox-title"><h5>新平台每日发货数据 </h5></div>
...
function getUrl(){
	var url = contextPath + "/report/daily_out_report/each_platform/grid?";
	var platform = $("#platform").val();
	if(platform !== "all"){
		url +=	"platform=" + platform;
	}else{
		url += "platform=newAndAll";
	}
...
function exportGridData(){
	var fileName = "新平台每日发货数据 " + new Date().getTime() + ".csv";
	var title = [ '日期','平台', '发货单数', '客单价', '发货收入', '退款', '成本', '毛利', '运费', '平台费用', '包材费', '订单执行费', '运营费', '边际利润', '净利', '税后综合利润率'];
 ...
 (function(){
	...
	common.grid({
		...
		title:"新平台每日发货数据 "
		,url:getUrl()
		,colNames:[ '日期','平台', '发货单数', '客单价', '发货收入', '退款', '成本', '毛利', '运费', '平台费用', '包材费', '订单执行费', '运营费', '边际利润', '净利', '税后综合利润率']
