$(function () {
	...
	jQuery("#list2").jqGrid(
			{
				url : contextPath + '/report/ebayoverseascategory/grid?business=Ebay_US',//组件创建完成之后请求数据的url
				datatype : "json",//请求数据返回的类型。可选json,xml,txt
				colNames : [ '分类', '原始sku', '日期（day）', '订单数' ,'数量' ,'订单金额_美元'],//jqGrid的列显示名字
				...
				pager : '#pager_category_us',//表格页脚的占位符(一般是div)的id
	...
	/*创建jqGrid的操作按钮容器*/
	/*可以控制界面上增删改查的按钮是否显示*/
	jQuery("#list2").jqGrid('navGrid', '#pager_category_us', {edit : false,add : false,del : false});
...
function refreshGridData(){
	...
	var et = $('#end_date').val();
	var url = contextPath + '/report/ebayoverseascategory/grid?st=' + st + "&et=" + et+ "&oldsku=" + oldsku+ "&category=" + category+ "&business=Ebay_US";
